...
export default function PageHero({
  ...
  return (
    <section className="relative isolate overflow-hidden pt-12 text-[#F8FAFC] md:pt-16" style={{ background: 'radial-gradient(circle at top right, rgba(47,128,237,0.28), transparent 35%), linear-gradient(180deg, #07111F 0%, #0B1B2B 100%)' }}>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 pb-6 pt-12 md:pb-8 md:pt-16">
        <div className={aside ? 'grid gap-10 md:grid-cols-[minmax(0,1fr)_360px] md:items-start' : ''}>
          <div>
            {eyebrow ? (
              <div className="inline-flex items-center gap-3 rounded-full border border-white/[0.12] bg-[#101E2F] px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-[#CBD5E1]">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              ...
              {secondaryCta?.href ? (
                <a href={secondaryCta.href} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center rounded-full border border-white/15 bg-[#101E2F] px-7 py-3.5 text-base font-semibold text-[#F8FAFC] transition hover:bg-[#0B1B2B] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#38BDF8]/70"><span>{secondaryCta.label}</span></a>
              ) : secondaryCta ? (
                <Link to={secondaryCta.to} className="inline-flex items-center justify-center rounded-full border border-white/15 bg-[#101E2F] px-7 py-3.5 text-base font-semibold text-[#F8FAFC] transition hover:bg-[#0B1B2B] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#38BDF8]/70"><span>{secondaryCta.label}</span></Link>
              ) : null}
              {showBooking ? <a href={BOOKING_URL} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center rounded-full border border-white/15 bg-[#101E2F] px-7 py-3.5 text-base font-semibold text-[#F8FAFC] transition hover:bg-[#0B1B2B] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#38BDF8]/70"><span>{bookingLabel}</span></a> : null}
            </div>
          </div>

          {aside ? (
            <div className="rounded-3xl border border-white/[0.12] bg-[#101E2F] p-7 shadow-[0_22px_70px_-48px_rgba(0,0,0,0.9)] md:p-8">
              ...
              {aside.items?.length ? (
                <div className="mt-7 grid gap-3 sm:grid-cols-3 md:grid-cols-1">
                  {aside.items.map((it) => (
                    <div key={it.k} className="rounded-2xl border border-white/[0.12] bg-[#0B1B2B] px-4 py-3">

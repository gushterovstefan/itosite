          ...
          <Card variant="steel" revealDelay={0.10}>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-5">
              {l.phases.map((phase, i) => (
                <div key={phase} className="rounded-2xl border border-white/10 bg-[#101E33] p-3">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <Card variant="brand" revealDelay={0.04}>
            <div className="grid gap-3 md:grid-cols-5">
              {l.phases.map((phase, i) => (
                <div key={phase} className="relative rounded-2xl border border-white/10 bg-[#101E33] p-4 text-center">
                  <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-brand-200/70 text-xs font-bold text-[#F8FAFC]">{i + 1}</div>
          ...
          <Card variant="amber" revealDelay={0.10}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {page.standards.map((item) => (
                <span key={item} className="rounded-full border border-white/10 bg-[#101E33] px-3 py-1 text-xs font-semibold text-[#94A3B8]">{item}</span>
              ))}
            </div>
          </Card>
        </div>
